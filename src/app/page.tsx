...
import { toast } from "sonner"; // අලුතින් එකතු කළා
import { 
...
const Map = dynamic(() => import("@/components/Map").then((mod) => mod.default), { 
  ssr: false, 
  loading: () => <div className="h-full w-full bg-slate-50 flex items-center justify-center animate-pulse font-black text-slate-300 uppercase tracking-widest">Map Loading...</div>
});
...
export default function Home() {
  ...
  const sendSOS = async (type: string) => {
    if (!userLoc) { toast.error("GPS location not available"); return; }
    setLoading(true);
    const toastId = toast.loading(`Sending ${type} request...`);
    ...
    try {
      ...
      toast.success("SOS signal sent! Volunteers notified.", { id: toastId });
  ...
  return (
    <main className="flex h-screen w-full bg-slate-50 overflow-hidden font-sans text-slate-900 antialiased">
      <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="w-full md:w-[400px] bg-white shadow-2xl flex flex-col z-30 border-r border-slate-100">
        <div className="p-8 bg-slate-900 text-white relative">
          <div className="flex justify-between items-center mb-6">
            ...
            {session ? (
              <button onClick={() => { supabase.auth.signOut(); toast.info("Logged out"); }} className="p-2 bg-slate-800 rounded-xl hover:bg-rose-500"><LogOut className="w-4 h-4" /></button>
            ) : (
              <button onClick={() => router.push('/login')} className="bg-slate-800 px-3 py-1.5 rounded-xl hover:bg-blue-600 transition-all text-[9px] font-black uppercase flex items-center gap-2 tracking-widest"><LogIn className="w-3 h-3" /> Login</button>
            )}
          </div>

          <div className="flex flex-col gap-2">
            <button onClick={() => setShowHeatmap(!showHeatmap)} className={`w-full py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${showHeatmap ? 'bg-rose-500 border-rose-400 text-white shadow-lg' : 'bg-slate-800 border-slate-700 text-slate-400 hover:text-white'}`}>
            ...
            {appLoading ? (
              ...
              <motion.div key={viewMode} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
                {viewMode === 'emergencies' && (
                  <>
                    <div className="relative mb-4"><Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300" /><input type="text" placeholder="Search incidents..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white border border-slate-100 py-3 pl-11 pr-4 rounded-2xl text-xs font-bold focus:ring-2 focus:ring-blue-500/10 transition-all shadow-sm" /></div>
                    {filteredList.map((req) => (
                      <motion.div layout key={req.id} onClick={() => setSelectedLoc([req.latitude, req.longitude])} className="p-5 rounded-[2rem] border-2 border-slate-100 bg-white hover:border-rose-300 transition-all cursor-pointer shadow-sm shadow-slate-200/50">
                ...
                {viewMode === 'safezones' && safeZones.map((zone) => <motion.div layout key={zone.id} onClick={() => setSelectedLoc([zone.latitude, zone.longitude])} className="p-6 rounded-[2rem] border-2 border-slate-100 bg-white hover:border-emerald-300 transition-all cursor-pointer shadow-sm group"><span className="text-[9px] font-black px-2 py-1 bg-emerald-50 text-emerald-600 rounded-lg mb-2 inline-block uppercase">{zone.type}</span><p className="text-sm font-black text-slate-700 uppercase">{zone.name}</p></motion.div>)}
        ...
        <div className="p-6 bg-white border-t border-slate-100">
          {!showOptions ? (
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setShowOptions(true)} className="w-full h-20 bg-rose-600 text-white text-lg font-black rounded-[2.5rem] shadow-xl shadow-rose-200 hover:bg-rose-700 transition-all flex items-center justify-center gap-3 animate-pulse uppercase tracking-widest">
            ...
            <div className="space-y-4 animate-in slide-in-from-bottom-5 duration-300">
              <label className="flex items-center justify-center gap-3 bg-slate-50 border-2 border-dashed border-slate-200 p-4 rounded-[2rem] cursor-pointer hover:bg-slate-100 transition-all group">
                ...
                <input type="file" accept="image/*" capture="environment" className="hidden" onChange={(e) => {setFile(e.target.files?.[0] || null); toast.success("Photo captured!"); }} />
              </label>
              <div className="grid grid-cols-3 gap-2">
                {['Medical', 'Food', 'Rescue'].map(t => (
                  <button key={t} disabled={loading} onClick={() => sendSOS(t)} className="flex flex-col items-center bg-slate-50 border-2 border-slate-100 p-4 rounded-3xl transition-all hover:bg-rose-50 hover:border-rose-100 group">
                    ...
                    <span className="text-[9px] font-black uppercase text-slate-600">{t}</span>
                  </button>
                ))}
      ...
      <div className="flex-1 relative overflow-hidden"><Map onRequestsUpdate={(data: any) => setRequests(data)} selectedLocation={selectedLoc} filter={activeFilter} safeZones={safeZones} showHeatmap={showHeatmap} isVolunteer={!!session} showShelterRoute={showShelterRoute} /></div>
    </main>
  );
}